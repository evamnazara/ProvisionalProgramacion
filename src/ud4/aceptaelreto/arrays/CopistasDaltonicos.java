...
public class CopistasDaltonicos {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("Introduce el tamaño del cuadro (filas y columnas):");
            ...
            // Leer el cuadro original en un array unidimensional
            System.out.println("Introduce el cuadro original:");
            char[] cuadro = new char[filas * columnas]; // Un array para todo el cuadro
            sc.nextLine(); // Consumir el salto de línea
            for (int i = 0; i < filas; i++) {
                String fila = sc.nextLine();
                for (int j = 0; j < columnas; j++) {
                    cuadro[i * columnas + j] = fila.charAt(j); // Almacenar en el array unidimensional
                }
            }
            ...
            for (int i = 0; i < numCopistas; i++) {
                ...
                for (int j = 0; j < cuadro.length; j++) {
                    ...
                    if (cuadro[j] == colorNoDistinguido) {
                        cuadro[j] = colorSustituido;
                    }
                }
            }
            ...
            for (int i = 0; i < filas; i++) {
                // Imprimir cada fila del cuadro, extrayendo las columnas del array
                // unidimensional
                for (int j = 0; j < columnas; j++) {
                    System.out.print(cuadro[i * columnas + j]);
                }
                System.out.println();
            }
        }
